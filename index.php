   ...
   if ($show_bar){
      ...
      <script>
         jQuery('document').ready(function($){
            ...
            <div style="width: 50px;height: 50px;display: flex;justify-content: center;align-items: center;border-radius: 50%;background: <?php echo buddyboss_theme_get_option( 'accent_color' );?>" class="trophy">
               <img src="<?php echo $rank_img;?>" style="width: 50px">
            </div>
            <div style="display: flex;flex-direction: column;min-width: 200px;padding-left: 15px;">
               ...
               <div style="padding-top: 5px">
                  <div style="width:100%;height: 8px;background-color: #EBF0F4;border-radius: 10px ">
                     <div class="progress_bar" style="width:<?php echo $completion?>%;height: 8px;border-radius: 10px; background: <?php echo buddyboss_theme_get_option( 'accent_color' );?>">
                     </div>
                  </div>
               </div>
            </div>
         </div>
         <a style="display: flex;align-items: center; margin-right: 35px" href="<?php echo $redeem_screen;?>" data-balloon-pos="down" data-balloon="Earn coins to unlock goodies" class="coins">
            <div style="display: flex;justify-content: center;align-items: center;border-radius: 50%;">
               <img src="<?php echo $coins_img;?>" style="max-width:40px;border:none;">
            </div>
            <div style="font-size: 16px;padding-left: 10px;font-weight: bold;" >
               <?php echo $current_coins;?>
            </div>
         </a>`);
         })
      </script>
...
function pb_digital_submenu_page(){
   ...
   if(function_exists("gamipress_get_rank_type")){
      ...
      <script>
      jQuery(document).ready(function($){
         $('.progress_bar').val('<?php echo get_option( 'pbd_progress_bar',0);?>');
         $('.rank').val('<?php echo get_option( 'pbd_rank_type');?>');
         
         $('.points').val('<?php echo get_option( 'pbd_points_type');?>');
         $('.coins').val('<?php echo get_option( 'pbd_coins_type');?>');
         $('.redeem_page').val('<?php echo get_option( 'pbd_redeem_page');?>');
      });
      </script>
      <?php
   }
...
if(!function_exists("pbd_courses_path")){
    ...
    function pbd_courses_path ($args) {
        ...
        if(!empty($args["ring-color-active"])){
            ?>
            .timeline-article.active .content-course__lesson {
                border-color: <?php echo $args["ring-color-active"]?> !important;
            }
            <?php
        }
        if(!empty($args["ring-color-completed"])){
            ?>
            .timeline-article.completed .content-course__lesson {
                border-color: <?php echo $args["ring-color-completed"]?> !important;
            }
            <?php 
        }
        ...
        if(!empty($args["ring-color"])){
            ?>
            #courses-timeline .content-course__lesson {
                border: 10px solid <?php echo $args["ring-color"]?>;
            }
            <?php 
        }
        ...
        if(!empty($args["path-color-completed"])){
            ...
            .timeline-article::before,
            #courses-timeline .timeline-start::after {
                background: <?php echo $args["path-color-completed"]?>;
            }
            <?php
        }
        ...
        if(!empty($args["path-color"])){
            ?>
            #courses-timeline .courses-center-line{
                background: <?php echo $args["path-color"]?>;
            }
            <?php
        }
        ...
        if(!empty($args["progress-color"])){
            ?>
            #courses-timeline .content-course__progressbar-inner{
                background: <?php echo $args["progress-color"]?>;
            }
            <?php
        }
        ...
        ?>
        #courses-timeline .btn-start {
            <?php if(!empty($args["start-background-color"])){ ?> background: <?php echo $args["start-background-color"]?>; <?php }?>
            <?php if(!empty($args["start-ring-color"])){ ?>border: 10px solid <?php echo $args["start-ring-color"]?>; <?php }?>
            <?php if(!empty($args["start-color"])){ ?>color: <?php echo $args["start-color"]?>; <?php }?>
        }
        #courses-timeline .timeline-end.completed {
            <?php if(!empty($args["completed-background-color"])){ ?> background: <?php echo $args["completed-background-color"]?>; <?php }?>
            <?php if(!empty($args["completed-ring-color"])){ ?>border: 10px solid <?php echo $args["completed-ring-color"]?>; <?php }?>
        }

        <?php if(!empty($args["completed-color"])){ ?>
        #courses-timeline .timeline-end.completed svg path {
            stroke: <?php echo $args["completed-color"]?>;
        }
        <?php }?>
        ...
        if(!empty($args["checkmak-color"])){
            ?>
            .path-inner .checkmark {
                stroke: <?php echo $args["checkmak-color"]?>;
            }
        <?php
        }
        /*
        ...
        <div class="path">
            <div class="path-inner">
                ...
                <section id="courses-timeline">
                    ...
                    <?php echo ($incomplete) ? 'completed' : '';?>"><button class="btn-start">Start</button></div>
                    ...
                    <div class="courses-timeline-content">
                        ...
                        <?php echo ($completed) ? 'completed' : '';?>
                        <?php echo ($previous_completed) ? 'active' : '';?>"
                        onclick="window.location = '<?php echo get_permalink($lesson->ID);?>'"
                                ...
                                <div class="timeline-article--inner">
                                    ...
                                    <div class="content-course__details">
                                        <div class="content-course__photo">
                                            <img src="<?php echo get_the_post_thumbnail_url( $lesson->ID);?>" alt="Yoga Beginner course">
                                        </div>
                                        <div class="content-course__info">
                                            <h3><?php echo $lesson->post_title;?></h3>
                                            <p><?php echo get_post_meta($lesson->ID, '_learndash_course_grid_short_description', true);?></p>
                                        </div>
                                    </div>
                                    <div class="content-course__progress">
                                        <div class="content-course__completed">
                                            <?php echo $topics_completed;?> of <?php echo $topics;?> lessons completed
                                        </div>
                                        <div class="content-course__progressbar">
                                            <div class="content-course__progressbar-inner" style="width:<?php echo $percent_completed;?>%"></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="timeline-end 
                        <?php echo ($completed) ? 'completed' : '';?>"><svg width="34" height="34" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M26.546 10.182c3 0 5.454-3.546 5.454-6.818h-5.454M7.454 10.182c-3 0-5.455-3.546-5.455-6.818h5.455M17 18.5v9.41M23.818 32H10.182m10.909-4.09h-8.182M26.545 8.817V2H7.456v6.818A9.487 9.487 0 0017 18.364a9.487 9.487 0 009.546-9.546z" stroke="#4D5E70" stroke-width="3" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/></svg></div>
...
add_shortcode("pbd_course_roadmap_curve", function($attr){
    ...
    .path-svg.completed svg .line, .path-svg svg.active .line {
        stroke: <?php echo $attr["path_active_background"]?>;
    }
    <?php 
    }
    if(!empty($attr["path_stroke_color"])){
        ...
        .path-svg.completed svg .stroke, 
        .path-svg svg.active .stroke {
            stroke: <?php echo $attr["path_stroke_color"]?>;
        }
        <?php
    }
    ...
    if(!empty($attr["start_button_color"])){
        ?>
        button.btn-start {
            background: linear-gradient(135deg, <?php echo $attr["start_button_color"]?> 0%, <?php echo $attr["start_button_color"]?> 100%);
            box-shadow: 0px 6px 0px <?php echo $attr["start_button_color"]?>, 0px 10px 8px rgb(46 66 90 / 15%);
        }
        <?php
    }
    ...
    if(!empty($attr["ring_default_color"])){
        ?>
        .path-svg .path-btn {
            border-color: <?php echo $attr["ring_default_color"]?>;
        }
        <?php
    }
    if(!empty($attr["ring_default_shadow_color"])){
        ?>
        .path-svg .path-btn {
            box-shadow: 0px 6px 0px <?php echo $attr["ring_default_shadow_color"]?>, 0px 10px 8px rgb(46 66 90 / 15%);
        }
        <?php
    }
    ...
    if(!empty($attr["ring_completed_color"])){
        ?>
        .path-svg.completed .path-btn {
            border-color: <?php echo $attr["ring_completed_color"]?>;
        }
        <?php
    }
    if(!empty($attr["ring_completed_shadow_color"])){
        ?>
        .path-svg.completed .path-btn {
            box-shadow: 0px 6px 0px <?php echo $attr["ring_completed_shadow_color"]?>, 0px 10px 8px rgb(46 66 90 / 15%);
        }
        <?php
    }
    ...
    if(!empty($attr["ring_active_color"])){
        ?>
        .path-svg.active .path-btn {
            border-color: <?php echo $attr["ring_active_color"]?>;
        }
        <?php
    }
    if(!empty($attr["ring_active_shadow_color"])){
        ?>
        .path-svg.active .path-btn {
            box-shadow: 0px 6px 0px <?php echo $attr["ring_active_shadow_color"]?>, 0px 10px 8px rgb(46 66 90 / 15%);
        }
        <?php
    }
    ...
    if(!empty($attr["title_color"])){
        ?>
        .path-lesson h3 {
            color: <?php echo $attr["title_color"]?>;
        }
        <?php
    }
    ...
    if(!empty($attr["finish_button_shadow_color"])){
        ...
        .path-svg.path-finish.active .path-btn {
            box-shadow: 0px 6px 0px <?php echo $attr["finish_button_shadow_color"]?>, 0px 10px 8px rgb(46 66 90 / 15%);
        }
        <?php
    }
    if(!empty($attr["finish_button_background_color"])){
        ?>
        .path-svg.path-finish.active .path-btn {
            background:<?php echo $attr["finish_button_background_color"]?>
        }
        <?php
    }
    ...
    if(!empty($attr["progress_completed_background"])){
        ?>
        .path-svg .courseprogress .checkmark__circle {
            stroke: <?php echo $attr["progress_completed_background"]?>;
            fill: <?php echo $attr["progress_completed_background"]?>;
        }
        <?php
    }
    ...
    if(!empty($attr["progress_default_background"])){
        ?>
        .path-svg .courseprogress span {
            background: <?php echo $attr["progress_default_background"]?>;
        }
        <?php
    }
    ...
    if(!empty($attr["progress_text_color"])){
        ?>
        .path-svg .courseprogress span {
            color: <?php echo $attr["progress_text_color"]?>;
        }
        .path-svg .courseprogress .checkmark__check{
            stroke:<?php echo $attr["progress_text_color"]?>;
        }
        <?php
    }
    ?>
    ...
    <div class="page-template-pt-pathdev">
        ...
        <div class="path">
            ...
            <div class="path-svg starting-path" data-iscomplete="1" data-progress="1">
                <button class="btn-start"><?php echo ($attr["start_button_text"]) ? $attr["start_button_text"]:"Start"?></button>
                <svg class="" viewBox="0 0 236 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path class="line" d="M234 10H2" stroke="<?php echo $default_path_color?>" stroke-width="20"/>
                    <path class="stroke" d="M234 10H2" stroke="<?php echo $default_path_color?>" stroke-width="3" stroke-linecap="round" stroke-dasharray="3 8"/>
                </svg>
            </div>
            <!-- end starting path -->
            <?php 
            ...
            foreach($course_lessons as $lesson){
                ...
                ?>
                <div 
                    data-progress="<?php echo ($percent_completed > 0) ? $percent_completed / 100:0?>"
                    data-href="<?php echo get_permalink($lesson_id)?>" 
                    data-iscomplete="<?php echo $iscomplete?>" 
                    data-topicscount="<?php echo $topics?>" 
                    data-percent="<?php echo $percent_completed?>" 
                    data-topicscompleted="<?php echo $topics_completed?>" 
                    data-j="<?php echo $j?>" class="path-svg <?php echo $class?> ">
                    <button class="path-btn">
                        <img src="<?php echo get_the_post_thumbnail_url($lesson_id)?>" alt="">
            ...
            ?>
            <div data-pathcount="<?php echo $path_count?>" 
                data-path-completed-count="<?php echo $path_completed_count?>" 
                class="path-svg <?php echo $classSvg[$j]["class"]?> path-finish" 
                data-iscomplete="<?php echo $iscomplete?>" >
                <button class="path-btn">
                    <?php 
                    if(!empty($attr["finish_image_url"])){
                        ?>
                        <img src="<?php echo $attr["finish_image_url"]?>" alt="" data-fromattr="1" />
                        <?php
                    }else{
                        ?>
                        <img src="<?php echo plugin_dir_url( __FILE__ )?>assets/img/finish.png" />
                        <?php
                    }
                    ?>
                ...
                <div class="path-lesson">
                    <h3>
                    <?php echo ($attr["finish_text"]) ? $attr["finish_text"]:"Finish"?>
                    </h3>
    ...
    jQuery(document).ready(function ($) {
        ...
        let time =  <?php echo $time?>;//500;
        let delay = <?php echo $delay?>;//1500;
